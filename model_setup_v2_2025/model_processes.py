...
import model_init as mi   # ← pulls in all the arrays & constants
...
def simulate_wb(time):
    ...
    # interception
    mi.interception[time] = min(P, 0.05*LAI)
    throughfall = P - mi.interception[time]

    #==========EVAPOTRANSPIRATION===================
    if mi.interception[time] >= mi.pET_k.iloc[time]:
        mi.total_evap[time] = mi.pET_k.iloc[time]
        ...
        remaining_PET = mi.pET_k.iloc[time] - mi.interception[time]
